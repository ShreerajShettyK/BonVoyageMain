...
export class TravelComponent {
  ...
  ngOnInit() {
    ...
    this.travellers = this.route.snapshot.paramMap.get('count')
    // console.log('ID:', this.travellers);
    this.numberOfTravellers = 1;
    
    this.enteredDate = new Date(this.date as any).toISOString().slice(0, 10) || new Date().toISOString().slice(0, 10);
    this.updateDates();
    this.packageForm = this.fb.group({
      numberOfTravellers: [this.travellers || 1, [Validators.required, this.travellerCountValidator()]],
      travelDate: [new Date(this.date as any).toISOString().slice(0, 10) || new Date().toISOString().slice(0, 10), [Validators.required, this.futureDateValidator()]]
    });
